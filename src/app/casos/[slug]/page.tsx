...
import type { Case, Victim, TimelineEvent, Suspect, Evidence, Source, Tribute, CaseImpact } from '@/lib/types'
import type { Metadata } from 'next'
import VictimProfile from './VictimProfile'
import TributeSection from './TributeSection'
import CaseImpactSection from './CaseImpactSection'
...
async function getCase(slug: string) {
  ...
  return {
    ...
    victims: caseData.victims.map(v => ({
      ...
      tributes: v.tributes as Tribute[],
    })) as Victim[],
    ...
    impact: caseData.impact ? {
      ...caseData.impact,
      mediaAdaptations: caseData.impact.mediaAdaptations
        ? JSON.parse(caseData.impact.mediaAdaptations)
        : null,
    } as CaseImpact : null,
  } as Case
}
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-wrap gap-4 text-sm" style={{ color: 'var(--muted)' }}>
                ...
                <span className="flex items-center gap-1">
                  ...
                  {formatNumber(caseData.viewCount)} vistas
                </span>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <div className="flex-1 min-w-0">
            {/* Victims - First section to prioritize victims over perpetrators */}
                ...
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    <h2 className="text-2xl font-bold">
                      {caseData.victims.length === 1 ? 'La Victima' : 'Las Victimas'}
                    </h2>
                    <p className="text-sm" style={{ color: 'var(--muted)' }}>
                      Recordando quienes eran, no solo lo que les paso
                    </p>
                  </div>
                </div>
                    ...
                    <div key={victim.id}>
                      ...
                      >
                        <TributeSection
                          ...
                          initialTributes={victim.tributes || []}
                          initialCount={victim.tributeCount}
                        />
                      </div>
                    </div>
                  ))}
            ...
            {caseData.timelineEvents && caseData.timelineEvents.length > 0 && (
              <section className="dossier-section mt-12">
                <h2 className="text-2xl font-bold mb-6">Línea Temporal</h2>
                        ...
                        <div>
                          ...
                          >
                            {suspect.role === 'CONFIRMED_PERPETRATOR' ? 'Culpable confirmado' :
                             suspect.role === 'SUSPECT' ? 'Sospechoso' : 'Persona de interés'}
                          </span>
                        </div>
                      </div>
                      {suspect.description && (
            ...
            {/* Case Impact */}
            {caseData.impact && (
              <CaseImpactSection impact={caseData.impact} />
            )}
          </div>
          ...
          <aside className="lg:w-80 flex-shrink-0">
            ...
            >
              ...
              <nav className="space-y-2 text-sm">
                {caseData.victims && caseData.victims.length > 0 && (
                  <a href="#" className="block py-1 transition-colors hover:opacity-70">Víctima(s)</a>
                )}
                {caseData.content && (
                  <a href="#" className="block py-1 transition-colors hover:opacity-70">Historia del Caso</a>
                )}
                {caseData.timelineEvents && caseData.timelineEvents.length > 0 && (
                  <a href="#" className="block py-1 transition-colors hover:opacity-70">Línea Temporal</a>
                )}
                {caseData.suspects && caseData.suspects.length > 0 && (
                ...
                {caseData.impact && (
                  <a href="#" className="block py-1 transition-colors hover:opacity-70">Legado e Impacto</a>
                )}
              </nav>
